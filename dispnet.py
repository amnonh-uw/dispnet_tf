...
class DispNet:
    def __init__(self):
        ...
        self.loss6 = self.l1loss(self.pr6, self.disp, self.loss_weights[5], "loss6")
        ...
        self.loss5 = self.l1loss(self.pr5, self.disp, self.loss_weights[4], "loss5")
        ...
        self.loss4 = self.l1loss(self.pr4, self.disp, self.loss_weights[3], "loss4")
        ...
        self.loss3 = self.l1loss(self.pr3, self.disp, self.loss_weights[2], "loss3")
        ...
        self.loss2 = self.l1loss(self.pr2, self.disp, self.loss_weights[1], "loss2")
        ...
        self.loss1 = self.l1loss(self.pr1, self.disp, self.loss_weights[0], "loss1")

        self.loss = self.loss1 + self.loss2 + self.loss3 + self.loss4 + self.loss5 + self.loss6
    ...
    def l1loss(self, pred, gt, weight, name):
        pred_shape = pred.get_shape()
        gt = tf.image.resize_images(gt, pred_shape[1:3])
        loss = tf.multiply(weight, tf.reduce_mean(tf.abs(pred - gt)), name=name)
